document.addEventListener("DOMContentLoaded", () => {
    ...
    function Book(title, author, pages) {
        ...
        this.info = function () {
            console.log(`${this.title} by ${this.author}, ${this.pages} pages, ${this.isRead ? "it was read" : "not read yet"}.`);
        };
    }
    const TheLordOfTheRings = new Book("The Lord of the Rings", "J.R.R. Tolkien", 299);
    ...
    function renderBookCards() {
        ...
        libraryArray.forEach((book) => {
            if (container) {
                ...
                newCard.append(document.createTextNode(`${book.isRead ? " -> gelesen" : " -> ungelesen"}`));
                container.append(newCard);
                const newButton = document.createElement("button");
                newButton.setAttribute("id", "deleteBookButton");
                newButton.innerHTML = "Remove";
                newButton.addEventListener("click", (event) => {
                    libraryArray = libraryArray.filter((filteredBook) => filteredBook.title !== book.title);
